...
for(const token of ['yasmin-gallery','auction-podium','old-cinema','cinema-screen','dima-apartments','nursery-garden','sonya-kitchen','octopus-institution','festival-stage'])assert(districtSource.includes(token),`district renderer missing ${token}`);
assert(!districtSource.includes('octopus-bank'),'renderer must not resurrect Octopus Bank');
...
for(const species of ['seagull','tropical-fish','squirrel'])assert(creatures.some(x=>x.species===species),`missing small-life species ${species}`);
