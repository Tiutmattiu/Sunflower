...
import {createSecuredClaim} from './npcEconomy.js';
...
export function yasminSecuredAdvance(w,borrowerId,collateralUnitId,{requestedPrincipal=Infinity,advanceRate=.6,term=5}={}){
 ensureBook(w);
 ...
 const recoveryValue=Math.max(1,Math.floor(value*.85));
 ...
 const result=createSecuredClaim(w,{borrowerId,lenderId:'yasmin',collateralUnitId,principal,face,dueDay:w.day+term,purpose:'yasmin_secured_advance'});
 if(!result.ok)return result;
 const claim=w.claims.find(c=>c.id===result.claimId);
 claim.recoveryValue=recoveryValue;claim.advanceRate=advanceRate;claim.appraisedKind=found.unit.kind;claim.privateCapital=true;
 emit(w,'yasmin_secured_advance',{claimId:claim.id,borrowerId,collateralUnitId,principal,face,recoveryValue,advanceRate});
