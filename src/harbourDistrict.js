...
export const DISTRICT_LANDMARKS = Object.freeze([
  ...
  {id:'festival-stage',ontologyId:'music-festival',kind:'music-festival',x:930,y:900,w:320,h:150,label:'LIVE MUSIC'},
  ...
  // Octopus is one institution: clearing, payment and local-market data. public_clearing is the legacy runtime id.
...
export const DISTRICT_PROP_POINTS=Object.freeze({
  ...
  path:{x:145,y:700},orders:{x:1260,y:650},
});
...
const SIGNALS = Object.freeze([
  ...
  {id:'festival-sign',kind:'music-festival',x:930,y:820,label:'LIVE MUSIC • TONIGHT'},
]);
...
function liveDistrictFacts(world,day){
  ...
  return {auctionPreview,auctionDay,barBusy,wongBusy:wongLoad>=3,octopusBusy:openOrders>=2,toadGathering};
}
...
export function districtSignals(world={}) {
  ...
  return SIGNALS
    .filter(signal=>signal.kind!=='music-festival'||state.festivalDay)
    .map(signal => ({
      ...signal,
      muted: state.commercialQuiet && ['wong-services','pizza-shop','deli','barber','massage'].includes(signal.kind),
      emphasized:
        ((state.musicNight||state.barBusy) && signal.kind === 'bar') ||
        (state.festivalDay && signal.kind === 'music-festival') ||
        ...
        (state.octopusBusy && signal.kind === 'octopus') ||
