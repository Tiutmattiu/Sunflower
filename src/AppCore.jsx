...
import {HarbourDistrictBase} from './HarbourDistrictLayer.jsx';
...
function migrateSavedWorld(w){
 ...
 j.toadFavorUsed=Boolean(j.toadFavorUsed);
 if(!pg.sunflower?.owned&&!pg.knowledge.includes(K.juan_route)&&['planning','ready','won','complete'].includes(j.stage)){
 ...
 if(w.production&&w.actors.aspen&&!w.actors.aspen.inventory?.some(x=>x.kind==='Tiny Torque Wrench')){
  w.actors.aspen.inventory??=[];
  w.actors.aspen.inventory.push({unitId:`u${++w.nextUnit}`,kind:'Tiny Torque Wrench',owner:'aspen',age:0,costBasis:GOODS['Tiny Torque Wrench']?.value||0,source:'save_migration_tool',opened:false,remaining:1});
 }
 return w;
}
...
function Comic({scene,world,onClose,children}){
 ...
 const anchor=PLACES[scene.location]||PLACES.viewing_room;
 return <section ref={dialog} className="comic" role="dialog" aria-modal="true" aria-label={scene.title}>
  ...
  <div className="comic-panels"><article className={`comic-panel panel-${index}`}>
   <svg viewBox={index===1?"70 40 120 100":"35 10 155 140"} aria-hidden="true">
    <svg width="220" height="150" viewBox={`${anchor[0]-150} ${anchor[1]-90} 300 200`}><HarbourDistrictBase world={world}/></svg>
    {(scene.person||scene.people?.length>0)&&<g transform={`translate(${65+index*15} 122) scale(1.2)`}><Figure scale={.43} id={scene.people?.[index%scene.people.length]||scene.person||'aspen'} pose={index%2?'working':'neutral'}/></g>}
    <g transform="translate(120 65)"><ObjectArt kind={scene.object||'paper'}/></g>
   </svg>
   <p>{line}</p>
  </article></div>
...
export default function AppCore(){
 ...
 function execute(a){
  ...
  if(['joel_serve_juan','race_juan','juan_field_trip'].includes(a.id))protectedActors.push('joel','juan');
  ...
  if(['lime_inspect','sonya_attend','auction_bid','invite_toad_circle','race_juan','juan_field_trip'].includes(a.id)){
   ...
   setComic({
    ...
    title:{lime_inspect:'The short crate',sonya_attend:'Borrowed chairs',auction_bid:'The raised hand',invite_toad_circle:'A little circle',race_juan:'The wager',juan_field_trip:'Beyond the harbour'}[a.id],
 ...
 {comic&&<Comic scene={comic} world={world} onClose={()=>setComic(null)}>{comic.choices&&['disclose','ambiguous','misrepresent'].map(mode=><button className="response" key={mode} onClick={()=>{execute({id:'lime_represent',payload:{mode}});setComic(null)}}>{actionLine({id:'lime_represent',payload:{mode}},world)}</button>)}</Comic>}
