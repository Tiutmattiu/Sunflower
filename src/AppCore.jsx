...
export default function AppCore(){
 ...
 {tab==='places'&&<><section className={`scene card scene-${pg.location}`}><header><div><div className="eyebrow">Now · {LOCATIONS[pg.location].name}</div><h2>{LOCATIONS[pg.location].description}</h2></div></header><div className="scene-stage" aria-label={`${LOCATIONS[pg.location].name} scene`}>{scene.people.map(id=><button className={`hotspot person ${hotspot===id?'selected':''}`} onClick={()=>setHotspot(id)} key={id}><span className="figure" aria-hidden="true">●</span><strong>{pretty(id)}</strong><small>{world.actors[id].busy?world.actors[id].actions.at(-1)?.kind||'occupied':'present'}</small></button>)}{scene.objects.map(object=><button className={`hotspot object ${hotspot===object?'selected':''}`} onClick={()=>setHotspot(object)} key={object}><span className="object-mark" aria-hidden="true">◇</span><strong>{pretty(object)}</strong></button>)}{!scene.people.length&&!scene.objects.length&&<p>The room is quiet. Its traces remain.</p>}</div>{hotspot&&<div className="hotspot-focus"><strong>{pretty(hotspot)}</strong><p>{scene.people.includes(hotspot)?'Present here now.':'Ownership, custody, timing and location determine its legal uses.'}</p>{hotspotActions.length?hotspotActions.map((a,i)=><button className="btn" disabled={a.disabled} onClick={()=>act(a)} key={`${a.id}-${i}`}>{a.label}{a.reason&&<small>{a.reason}</small>}</button>):<small>No consequential use is available from this object now.</small>}</div>}{scene.traces.map((t,i)=><p className="scene-trace" key={`${t.day}-${i}`}>Earlier: {t.summary}</p>)}<div className="action-grid">{actions.map((a,i)=><button key={a.id+i} className="btn" disabled={a.disabled} title={a.reason||''} onClick={()=>act(a)}><kbd>{i+1}</kbd> {a.label}{a.reason&&<small>{a.reason}</small>}</button>)}</div>{pg.lastBlock&&<p className="blocked">{pg.lastBlock}</p>}</section><section className="place-grid" aria-label="Go somewhere else">{Object.entries(LOCATIONS).filter(([id])=>id!==pg.location).map(([id,x])=><button className="place-card" onClick={()=>{setHotspot(null);setWorld(w=>visitLocation(w,id))}} key={id}><strong>{x.name}</strong><span>{x.description}</span></button>)}</section></>}
